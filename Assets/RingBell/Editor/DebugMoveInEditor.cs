...
public class DebugMoveInEditor
{
	...
	private static float debugSpeed;
	private static float debugAcc = 14f;
	private static float debugSpeedMax=4;

	private static bool moveToggle;
	private static int toggleCountMax = 40;
	private static int toggleCount;

	private static GameObject paraObject;

	static DebugMoveInEditor()
	{
		...
		if (paraObject == null)
		{
			paraObject = GameObject.Find("ParallaxManager");
		}
	}
	...
	static void OnSceneView(SceneView sceneView)
	{
		...
		if (currentEvent.keyCode == KeyCode.Backslash)
		{
			toggleCount++;
			if (toggleCount > toggleCountMax)
			{
				Debug.Log("MoveToggle " + moveToggle);
		...
		if (moveToggle&&currentEvent.isKey
		...
		&& sceneView.camera.orthographic)
		{
			...
			switch (currentEvent.keyCode)
			{
				...
				case KeyCode.LeftBracket:
					debugSpeed = Mathf.Clamp(debugSpeed - debugAcc, -debugSpeedMax,0);
					_bolo = true;
	...
	private static void moveSelectedObjects(Vector3 direction, SceneView sceneView)
	{
		...
		var selection = paraObject;
		//apply the transformation to every selected gameObject
		transform((selection as GameObject).transform, new Vector3(0, debugSpeed, 0));
